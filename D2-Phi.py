...
from model.merge_phimoe import Merge_PhiMoESparseMoeBlock



cudnn.benchmark = False
parser = argparse.ArgumentParser(description='cfg')
parser.add_argument('--base_model_path', type=str, default="microsoft/Phi-3.5-MoE-instruct", help='Path to base model')
parser.add_argument('--expert_freq_path', type=str, default="cache/Phi-MoE-instruct_wikitext_20000_expert_frequencies.json", help='Path to expert frequencies')
parser.add_argument('--fisher_path', type=str, default="Model/fisher_Phi-MoE-instruct.pt", help='Path to fisher info')
parser.add_argument('--svd_scale_path', type=str, default="Model/SVD_scale_Phi-MoE-instruct_0-31_512.pt", help='Path to svd scale')
...
def runExperiment():
    ...
    def merge_model(base_model_path, expert_freq_path, svd_scale_path, fisher_path, delta_ratio, share_ratio, share_V, share_U, merge_method):
        ...
        for i in tqdm(range(len(model.model.layers)), desc="Merging layers"):
            Merge_MoE_Block = Merge_PhiMoESparseMoeBlock(model.config, share_ratio=share_ratio, 
    ...
    save_dir = f"{args['result_path']}/phimoe_delta-{args['delta_ratio']}-pp_ratio-{args['pp_ratio']}-shareV-{args['share_V']}"
    os.makedirs(save_dir, exist_ok=True)

    result_str = ppl_eval_sharing(model, tokenizer, experiment_name=f"D2-PhiMoE", datasets=['wikitext2', 'ptb', 'c4'], params_only=False, batch_size=16)
