...
class MixtralBlockSparseTop2MLP(nn.Module):
    def __init__(self, config: MixtralConfig, layer_order):
        super().__init__()
        self.config = config
        self.ffn_dim = config.intermediate_size
        self.hidden_dim = config.hidden_size
        ...
        self.w2 = nn.Linear(self.ffn_dim, self.hidden_dim, bias=False)
        self.w3 = nn.Linear(self.hidden_dim, self.ffn_dim, bias=False)
        # self.gate_proj = self.w1
        # self.down_proj = self.w2
        # self.up_proj = self.w3
    ...
    def probe_process(self, x, **kwargs):
        ...
        probe_out = self.act_fn(self.w1(probe, cal_mlp_probe_out_dim_metric=True)) * self.w3(probe, cal_mlp_probe_out_dim_metric=True)

        # calculate score
        if 'calib' in cfg['prune_method'] or 'runningmean' in cfg['prune_method'] or 'ema' in cfg['prune_method']:
            probe_out_dim_metric = self.pruning_module.cal_mlp_prune_metric(probe_out, self.w2.weight.data, cfg['prune_metric'], seq_selected_indices, global_metric_score_distribution=self.w2.get_global_metric_score_distribution(cur_batch_seq_len))
        else:
            probe_out_dim_metric = self.pruning_module.cal_mlp_prune_metric(probe_out, self.w2.weight.data, cfg['prune_metric'], seq_selected_indices)

        if 'flapratio' in cfg['prune_method']:
            probe_out_dim_indices, prune_out_dim_indices = self.pruning_module.sort_mlp_metric(probe_out_dim_metric, cfg['tc_multiple'], pruning_ratio=self.w2.pruning_ratio)
        else:
            probe_out_dim_indices, prune_out_dim_indices = self.pruning_module.sort_mlp_metric(probe_out_dim_metric, cfg['tc_multiple'])

        # extract matrix
        self.w1.prepare_async_weight(out_dim_indices=probe_out_dim_indices)
        self.w3.prepare_async_weight(out_dim_indices=probe_out_dim_indices)
        self.w2.prepare_async_weight(in_dim_indices=probe_out_dim_indices)
        ...
        if cfg['test_stage'] == True:
            ...
            if self.layer_order not in cfg['skip_layers']:
                if 'probe' in cfg['prune_method']:
                    ...
                    down_proj = self.w2(self.act_fn(self.w1(x, out_dim_indices=probe_out_dim_indices)) * self.w3(x, out_dim_indices=probe_out_dim_indices), in_dim_indices=probe_out_dim_indices)
                    return down_proj
...
class MixtralSparseMoeBlock(nn.Module):
    ...
    def __init__(self, config, layer_idx):
        ...
        self.experts = nn.ModuleList([MixtralBlockSparseTop2MLP(config, layer_idx) for _ in range(self.num_experts)])
