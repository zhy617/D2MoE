...
class Merge_PhiMoESparseMoeBlock(nn.Module):
    """
    ...
    @torch.no_grad()
    def merge_experts(self, module, svd_scale = None, hessian = None, scale_type='svdllm', preprocess_method = None, scale_mean_method = "freq"):
        ...
        if self.merge_method == "freq":
            ...
            fisher_scale_w1 = [0]*self.num_experts
            fisher_scale_w2 = [0]*self.num_experts
            fisher_scale_w3 = [0]*self.num_experts
            ...
            fisher_scale_w1 = [fisher_scale_w1[j] / sum(fisher_scale_w1) for j in range(self.num_experts)]
            fisher_scale_w2 = [fisher_scale_w2[j] / sum(fisher_scale_w2) for j in range(self.num_experts)]
            fisher_scale_w3 = [fisher_scale_w3[j] / sum(fisher_scale_w3) for j in range(self.num_experts)]
                
            self.Wmean1.weight.data = sum([module.experts[j].w1.weight * fisher_scale_w1[j] for j in range(self.num_experts)])
            self.Wmean2.weight.data = sum([module.experts[j].w2.weight * fisher_scale_w2[j] for j in range(self.num_experts)])
            self.Wmean3.weight.data = sum([module.experts[j].w3.weight * fisher_scale_w3[j] for j in range(self.num_experts)])

        elif self.merge_method == "random":
            torch.manual_seed(111)
            scale_alpha_1 = torch.rand(self.num_experts, device=module.experts[0].w1.weight.device)
            torch.manual_seed(222)
            scale_alpha_2 = torch.rand(self.num_experts, device=module.experts[0].w1.weight.device)
            torch.manual_seed(333)
            scale_alpha_3 = torch.rand(self.num_experts, device=module.experts[0].w1.weight.device)
            ...
            self.Wmean1.weight.data = sum([module.experts[i].w1.weight * scale_alpha_1[i] for i in range(self.num_experts)])
            self.Wmean2.weight.data = sum([module.experts[i].w2.weight * scale_alpha_2[i] for i in range(self.num_experts)])
            self.Wmean3.weight.data = sum([module.experts[i].w3.weight * scale_alpha_3[i] for i in range(self.num_experts)])
        ...
        if self.delta_ratio != 0:
            ...
            scale_w3_mean = None
            total_freq = 0
            if svd_scale is not None and scale_mean_method == "freq":
                for j in range(self.num_experts):
                    base_name = f"block_sparse_moe.experts.{j}."
            ...
            elif svd_scale is not None and scale_mean_method == "fisher":
                scaling_factor = 1.0
                scaling_factors_ft_list = self.expert_freq
                ...
                scale_w1_mean = sum([fisher_scale_w1[j] * svd_scale[base_name + "w1"] for j in range(self.num_experts)])
                scale_w2_mean = sum([fisher_scale_w2[j] * svd_scale[base_name + "w2"] for j in range(self.num_experts)])
                scale_w3_mean = sum([fisher_scale_w3[j] * svd_scale[base_name + "w3"] for j in range(self.num_experts)])
...
class meanW_deltaW(nn.Module):
    ...
    def forward(self, hidden_states):
        up = self.Wmean3(hidden_states) + self.deltaW3(hidden_states)
        gate = self.Wmean1(hidden_states) + self.deltaW1(hidden_states)
        return self.Wmean2(self.act_fn(gate) * up) + self.deltaW2(self.act_fn(gate) * up)



class meanW_deltaUV(nn.Module):
    def __init__(self, config, Wmean1, Wmean2, Wmean3, delta_ratio=1, delta_share_V=False, delta_share_U=False, 
                 experts_delta_u1_shared=None, experts_delta_v1_shared=None, 
                 experts_delta_u2_shared=None, experts_delta_v2_shared=None, 
                 experts_delta_u3_shared=None, experts_delta_v3_shared=None, shared_infer=False, layer_order=[]):
        super().__init__()
        self.intermediate_dim = config.intermediate_size
        self.hidden_dim = config.hidden_size
        self.dtype = torch.bfloat16
        ...
        elif delta_share_V == True and delta_share_U == True and delta_ratio != 0:
        ...
        self.pruning_module = HiddenRepresentationPruning(cfg, f'PhiMoE_mlp_{layer_order}', config)
