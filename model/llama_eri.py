...
class MixtralEriModel(torch.nn.Module):
    def __init__(self, model):
        super().__init__()
        self.model = model
        self.forward = self.model.forward
    ...
    def _create_new_module(self, target, key):
        ...
        new_module = Linear11(in_features, out_features, bias=bias, **kwargs)
...
def _get_target_modules(cfg):
    ...
    if cfg['merge_model']:
        if 'deepseek' in cfg['model_type']:
            target_modules = ['Wmean_gate','Wmean_down','Wmean_up']
        else:
            target_modules = ['Wmean1','Wmean2','Wmean3']
    else:
...
def _check_target_module_exists(target_modules, key):
    if cfg['merge_model']:
        if 'mixtral' in cfg['model_type'] or 'phi' in cfg['model_type'] or 'Mixtral' in cfg['model_type'] or 'Phi' in cfg['model_type']:
            match = re.match(r"model\.layers\.(\d+)\.block_sparse_moe\.(Wmean[123])", key)
        elif 'deepseek' in cfg['model_type'] or 'qwen' in cfg['model_type'] or 'Qwen' in cfg['model_type']:
            match = re.match(r"model\.layers\.(\d+)\.mlp\.(Wmean_gate|Wmean_down|Wmean_up)", key)
    else:
        match = re.match(r"model\.layers\.(\d+)\.block_sparse_moe\.experts\.\d+\.(w[123])", key)
    if isinstance(target_modules, str):
        target_module_found = re.fullmatch(target_modules, key)
    else:
        target_module_found = any(key.endswith(target_key) for target_key in target_modules) and match
...
class Linear11(nn.Linear, EriLayer):
    def __init__(
        self,
        in_features,
        out_features,
        **kwargs,
        ...
        if 'wandasp' in self.prune_metric:
            ...
            self.scaler_inp = torch.zeros(in_features, device=self.weight.data.device, dtype=torch.float32)
        ...
        if 'wandasp' in self.prune_metric:
            ...
            if cfg['calibration_stage'] == True:
                ...
                if cfg['pad_tokens'] is not None:
                    ...
                    norm_squared = torch.clamp(torch.linalg.vector_norm(inp, ord=2, dim=0) ** 2, max=cfg['data_type_max'])/seq_len
                    self.scaler_inp[update_indices] += (1 - momentum) * torch.clamp(norm_squared , max=cfg['data_type_max'])
        elif "flap" in self.prune_metric:
            self.baseline_inp = self.baseline_inp.to(cur_device)
    ...
    def update_global_metric_score_distribution(self, inp, update_indices):
        ...
        if 'wandasp' in self.prune_metric:
            ...
            if cfg['calibration_stage'] == True:
                ...
                norm_squared = torch.linalg.vector_norm(inp, ord=2, dim=0) ** 2
                mean_l2_norm = norm_squared / inp.size(0)
                denominator = (self.nsamples[update_indices] + batch_size)
                self.scaler_inp[update_indices] += mean_l2_norm / denominator
                # norm_squared = torch.linalg.vector_norm(inp, ord=2, dim=0) ** 2
                # denominator = (self.nsamples[update_indices] + batch_size)
    ...
    def get_global_metric_score_distribution(self, cur_seq_len=None):
        if 'wandasp' in self.prune_metric:
            ...
            return self.scaler_inp if cur_seq_len is None else self.scaler_inp
        elif "flap" in self.prune_metric:
    ...
    def get_weight(self, cur_default_stream=None):        
